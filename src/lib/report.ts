...
export function buildMarkdownReport(name: string, result: ScanResult): string {
  ...
  L.push("");
  L.push("## OWASP Top 10 (2021) & CWE compliance");
  const compliance = buildCompliance(result);
  L.push(...compliance.summary);
  L.push("");
  L.push("| Category | Checks automated | Findings | Worst severity | CWE references |");
  L.push("|---|---|---|---|---|");
  for (const c of compliance.categories) {
    L.push(
      `| ${c.category.id} ${c.category.title} | ${c.testedChecks}/${c.totalChecks} | ${c.findings.length} | ${c.worst ?? "—"} | ${c.cweRefs.map((r) => r.id).join(", ") || "—"} |`,
    );
  }
  L.push("");
  L.push("## Findings");
...
<h2>OWASP Top 10 (2021) &amp; CWE compliance</h2>
...
/** OWASP/CWE compliance section for the print-ready HTML export. */
function complianceSummary(result: ScanResult): string {
  const compliance = buildCompliance(result);
  const paragraphs = compliance.summary.map((p) => `<p>${p}</p>`).join("");
  const rows = compliance.categories
    .map(
      (c) =>
        `<tr><td>${c.category.id} ${c.category.title}</td><td>${c.testedChecks}/${c.totalChecks}</td><td>${c.findings.length}</td><td>${c.worst ?? "—"}</td><td>${c.cweRefs.map((r) => r.id).join(", ") || "—"}</td></tr>`,
    )
    .join("");
  return `${paragraphs}
<table style="border-collapse:collapse;font-size:13px;width:100%">
  <thead><tr>
    ${["Category", "Checks automated", "Findings", "Worst severity", "CWE references"]
      .map((h) => `<th style="text-align:left;border-bottom:1px solid #d4d4d8;padding:6px 8px">${h}</th>`)
      .join("")}
  </tr></thead>
  <tbody>${rows}</tbody>
</table>`;
